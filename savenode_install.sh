...
libminiupnpc-dev libgmp3-dev ufw pkg-config libevent-dev  unzip libzmq5 libdb5.3++-dev >/dev/null 2>&1
if [ "$?" -gt "0" ];
  then
...
bsdmainutils libdb4.8++-dev libminiupnpc-dev libgmp3-dev ufw pkg-config libevent-dev unzip libzmq5 libdb5.3++-dev"
 exit 1
fi
clear
}
